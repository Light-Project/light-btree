...
bnode_alloc(struct btree_root *root)
{
    struct btree_layout *layout = root->layout;
...
int btree_update(struct btree_root *root, uintptr_t *key,
                 void *value)
{
    struct btree_layout *layout = root->layout;
    struct btree_node *node;
...
static int btree_extend(struct btree_root *root)
{
    struct btree_node *node;
    unsigned int index;
...
static int insert_level(struct btree_root *root, unsigned int level,
...
int btree_insert(struct btree_root *root, uintptr_t *key,
                 void *value)
{
    if (!value)
        return -EINVAL;
